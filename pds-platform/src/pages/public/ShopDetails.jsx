...
export default function ShopDetails() {
  ...
  useEffect(() => {
    if (location.state?.shop) return;
    fetchShopById(id).then(s => { setShop(s); setLoading(false); });
  }, [id, location.state?.shop]);

  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <span className="w-8 h-8 border-2 border-green-700/30 border-t-green-700 rounded-full animate-spin" />
      </div>
    );
  }
